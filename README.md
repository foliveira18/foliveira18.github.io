- **Current:** Head of Division (Analytics, Data & Modelling) — OECD, Trade & Agriculture  
- **Interests:** decision support, behavioral design, applied analytics, product demos, rapid prototyping  

I build practical, lightweight tools that turn messy real-world decisions into small, trackable actions.

My work connects **operations research, artificial intelligence, and economic policy analysis** to support decision-making under uncertainty in complex systems such as supply chains, energy markets, and digital platforms.

---

## Publications & Research

Peer-reviewed publications in operations research, energy economics, decision science, and information systems.

- **Academic papers repository:**  
https://foliveira18.github.io/published_articles/

- **Risk Analytics Book** — draft chapters of the book *Risk Analytics: Machine Learning and Optimization for Data-Driven Decision Making*  
https://foliveira18.github.io/risk-analytics-book/

Research profiles:

- Google Scholar: <https://scholar.google.com/citations?hl=en&user=iO2v-HgAAAAJ>  
- ORCID: <https://orcid.org/0000-0003-4357-1607>

---

## Writing

I also develop longer-form essays and book manuscripts exploring the conceptual foundations of decision-making, risk, and complex systems.

Substack publications:

- **Risk Analysis Notebook** — research notes and draft chapters from a book project on risk analysis, decision theory, and quantitative modelling of uncertainty.  
  https://riskanalysis.substack.com

- **Become Tomorrow** — essays exploring the philosophical and scientific implications of artificial intelligence, technological evolution, and the future of human decision-making.  
  https://becametomorrow.substack.com
---

## Projects

I also build lightweight prototypes for decision-support and behavioural tools.

Example:

**Pathfinder — decision-support micro-coach**

Landing page  
https://foliveira18.github.io/pathfinder/

Demo  
https://pathfinder-demo-eight.vercel.app

Repositories

- https://github.com/foliveira18/pathfinder  
- https://github.com/foliveira18/pathfinder-demo

---

## Contact

Email  
foliveira18@gmail.com  

LinkedIn  
<https://www.linkedin.com/in/foliveira18/>

---

*Disclaimer: views expressed in personal writing and projects are my own.*

